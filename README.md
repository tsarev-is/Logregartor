...
целостности. Инициализирован [Analytics](src/Analytics/README.md): отдельный
Python CLI с первым правилом по длительности создания VM и JSON-отчётом.
Подключение аналитики к ClickHouse и UI, корреляция и векторный поиск ещё впереди.
Компоненты проекта:
...
- **Аналитика** — модуль `src/Analytics`: детерминированные правила над событиями LogParser; начальная версия читает JSONL и обнаруживает превышение явно заданного порога длительности создания VM.
- **AI-объяснение** — планируемые гипотезы о причинах сбоев с опорой на найденные логи (RAG), после обнаружения и корреляции событий.
