...
очищать базу или повторно импортировать данные не требуется. Сервис `clickhouse-init`
создаёт схемы LogParser/Analytics и [агрегаты UI](UI_AGGREGATES.md) до инициализации
MCP. Загрузка событий выполняется отдельно конвейером LogParser.
