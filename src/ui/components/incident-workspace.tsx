    ...
    <main className="app-shell">
      <aside className={`sidebar ${mobileNav ? "sidebar-open" : ""}`}>
        ...
        <nav className="primary-nav">
          ...
          <button type="button" className={`nav-item ${mainView === "services" ? "active" : ""}`} aria-label="Services" onClick={() => { setMainView("services"); setMobileNav(false); }}><Server size={18} /> Services <span className="nav-count">{services.length || "—"}</span></button>
          ...
          <button type="button" className={`nav-item ${mainView === "logs" ? "active" : ""}`} aria-label="Live logs" onClick={() => openLogs({ query: "", component: "" })}><Database size={18} /> Live logs</button>
          ...
          <section className="incident-column" id="incidents">
            ...
            {card && timeline && <>
              <section className="panel cause-panel" id="incident-detail">
                ...
                {incidentServices.length > 0 && <div className="service-row" aria-label="Observed services in this evidence snapshot"><span>Observed services</span><div>{incidentServices.map((service) => <button type="button" className="service-chip" data-service={service} key={service} onClick={() => openLogs({ component: service, query: card.instance_id })}>{service}</button>)}</div></div>}
              ...
              <section className="panel timeline-panel" id="timeline"><div className="panel-title-row"><div><span className="panel-kicker">EVIDENCE TIMELINE</span><h2>Chronological source events for this build</h2><p className="timeline-context">VM <code>{card.instance_id}</code> · source <code>{short(card.source_sha256)}</code> · {timeline.events.length} timestamped event{timeline.events.length === 1 ? "" : "s"}</p></div></div>
                <p className="timeline-note">UTC order shows what was observed, not a causal relationship. Select an event to inspect its exact source record.</p>
                <div className="evidence-timeline">{timeline.events.map((event) => <button className="evidence-row" key={event.event_id} onClick={() => openEvidence(event.event_id)} aria-label={`Open source record ${event.event_id}`}><time dateTime={event.event_time ?? undefined}>{event.event_time}</time><span className="event-main"><strong>{event.component ?? "Unknown component"}</strong><span>{event.message}</span></span><small>Source line {event.line_start} · {event.parse_status} · event {short(event.event_id)}</small></button>)}</div>
                {timeline.undated_events.length > 0 && <><h3 className="undated-heading">Records without timestamps</h3><p className="timeline-note">These records remain available as evidence but cannot be placed in chronological order.</p>{timeline.undated_events.map((event) => <button className="evidence-row" key={event.event_id} onClick={() => openEvidence(event.event_id)} aria-label={`Open source record ${event.event_id}`}><span className="event-main"><strong>{event.component ?? "Unknown component"}</strong><span>{event.message}</span></span><small>Source line {event.line_start} · {event.parse_status} · event {short(event.event_id)}</small></button>)}</>}
              </section>
            </>}
