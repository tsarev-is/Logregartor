...
function formatUtc(timestampMs: number | null) {
...
export function LiveLogExplorer({ initialQuery = "" }: { initialQuery?: string }) {
  ...
  }, [debouncedQuery, level, source, offset, refreshToken]);
    ...
    <section className={styles.explorer} aria-label="Live ClickHouse log explorer">
      <header className={styles.header}>
        <div>
          ...
          <p>Finalized events from the <code>logs.log_events</code> view. Results are bounded and read only.</p>
        </div>
        <button type="button" className={styles.refresh} onClick={() => setRefreshToken((value) => value + 1)} disabled={loading}>
      ...
      <div className={styles.controls}>
        <label className={styles.search}>
          ...
          <div><Search size={15} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="message, event, request or instance ID" /></div>
        </label>
      ...
      <div className={styles.status} aria-live="polite">
        <span>{loading ? "Loading ClickHouse events…" : error ? "Log query failed" : range}</span>
        {page && <span>Dataset: <strong>{page.dataset}</strong> · page size {page.limit}</span>}
      </div>
      ...
      {!error && !loading && page?.events.length === 0 && <section className={styles.empty}><h2>No matching events</h2><p>Change the search text or filters.</p><button type="button" onClick={reset}>Reset filters</button></section>}

      {!error && page && page.events.length > 0 && <>
        <div className={styles.table} role="table" aria-label="Real OpenStack log events">
          <div className={styles.tableHead} role="row"><span>UTC timestamp</span><span>Level</span><span>Component</span><span>Source</span><span>Message</span></div>
          {page.events.map((event) => <button type="button" role="row" aria-selected={selected?.eventId === event.eventId} className={`${styles.row} ${selected?.eventId === event.eventId ? styles.selected : ""}`} key={event.eventId} onClick={() => setSelected(event)}>
            <time>{formatUtc(event.timestampMs)}</time>
      ...
      {selected && <section className={styles.inspector} aria-label={`Raw log event ${selected.eventId}`}>
        ...
        <dl>
          <div><dt>Timestamp</dt><dd>{formatUtc(selected.timestampMs)}</dd></div>
